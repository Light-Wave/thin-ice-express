...
func load_level(level_num: int) -> void:
	...
	match current_level:
		1:
			...
			if locomotive and locomotive.has_method("set_train_type"):
				locomotive.set_train_type(0) # 0 = Vintage Steam
			if ice_generator:
		...
		2:
			...
			if locomotive and locomotive.has_method("set_train_type"):
				locomotive.set_train_type(1) # 1 = X2000 Series
			if ice_generator:
		...
		4:
			...
			if locomotive and locomotive.has_method("set_train_type"):
				locomotive.set_train_type(2) # 2 = Bullet Train
			if ice_generator:
