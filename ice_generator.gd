...
func _process(_delta: float) -> void:
	...
	var current_row := int(floor(player_train.global_position.y / tile_size))
	var current_col := int(floor(player_train.global_position.x / tile_size))
...
func _generate_initial_grid() -> void:
	...
	for row_offset in range(-3, spawn_rows_ahead):
		for col_offset in range(-int(spawn_cols_wide / 2.0), int(spawn_cols_wide / 2.0) + 1):
			var grid_pos := Vector2i(start_col + col_offset, start_row + row_offset)
			
			# Keep starting tiles solid ice
			var is_start_zone := abs(row_offset) <= 2
...
func _update_grid_around(center_col: int, center_row: int) -> void:
	...
	var min_row := center_row - 3
	var max_row := center_row + spawn_rows_ahead
	var min_col := center_col - int(spawn_cols_wide / 2.0)
	var max_col := center_col + int(spawn_cols_wide / 2.0)
	...
	var player_y := player_train.global_position.y
...
func _spawn_tile_at(grid_pos: Vector2i, is_safe_start: bool) -> void:
	...
	var tile := ice_tile_scene.instantiate() as IceTile
